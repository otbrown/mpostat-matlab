...
function [densityMatrix, varargout] = DMRebuild(dmps, varargin)
    ...
    for braState = 0 : 1 : SPACE - 1
        for ketState = 1 : 1 : SPACE
            stateDex = braState * SPACE + ketState;
            braStr = dec2base(braState, HILBY, LENGTH);
            ketStr = dec2base(ketState - 1, HILBY, LENGTH);

            coefft = 1;
            for site = 1 : 1 : LENGTH
                bra = str2double(braStr(site)) + 1;
                ket = str2double(ketStr(site));
                mpsLoc = ket * HILBY + bra;
                coefft = coefft * dmps{site}(:, :, mpsLoc);
            end
