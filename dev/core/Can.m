...
function [ cdmpo ] = Can(dmpo, route, direction)
    ...
    if strcmpi(direction, 'L')
        ...
        elseif route(1) > route(end)
            msgID = 'Can:LCan:BadRoute';
            msg = sprintf('This route appears to be for the wrong direction. Left canonisation must be performed moving from the first site to the last.');
            ME = MException(msgID, msg);
            throw(ME);
        end
        ...
        for site = route
            ...
            [cdmpo{site}, cdmpo{site+1}] = LCan(siteTensor, nextSiteTensor, ...
                                                HILBY, ROW_SIZE, COL_SIZE, ...
                                                NEXT_COL);
        end
    elseif strcmpi(direction, 'R')
        ...
        elseif route(1) < route(end)
            msgID = 'Can:RCan:BadRoute';
            msg = sprintf('This route appears to be for the wrong direction. Right canonisation must be performed moving from the last site to the first.');
            ME = MException(msgID, msg);
            throw(ME);
        end
        ...
        for site = route
            ...
            [cdmpo{site}, cdmpo{site-1}] = RCan(siteTensor, nextSiteTensor, ...
                                                HILBY, ROW_SIZE, COL_SIZE, ...
                                                NEXT_ROW);
        end
    else
        msgID = 'Can:BadDirection';
        msg = sprintf('direction should be case insensitive character L or R. Supplied direction was %s', direction);
        ME = MException(msgID, msg);
        throw(ME);
    end
end
